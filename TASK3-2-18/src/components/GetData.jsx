  ...
  const handleDelete = async (id, timestamp) => {
    try {
      ...
      if (new Date(timestamp) > new Date(lastSyncTimestamp)) {
        ...
        if (!existingRecord) {
          const deletedRecord = { _id: id, timestamp: timestamp }; // Replace with your actual record structure
          await deletedObjectStore.add(deletedRecord);
          console.log('Added record to forms_to_be_sync object store');
        } else {
                ...
                <TableCell>
                  <div style={{ display: 'flex' }}>
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (navigator.onLine) {
                          handleDelete(entry._id['$oid'] , entry.data.timestamp);
                        } else {
                          handleDelete(entry._id, entry.data.timestamp);
                        }
                      }}
